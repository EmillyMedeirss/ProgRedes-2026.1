...
my_sock.connect(('viacep.com.br', 80))

my_sock.send (b"GET /ws/rn/natal/morais/json HTTP/1.1\r\n"+
...
if len_data != -1:
    ...
    data = data[pos2NL+4:]           #remove os cabecalhos e mantem apenas os dados da resposta
    pos = data.find(b"\r\n")         #localiza o tamanho do primeiro chunked q fica apos a linha em branco
    chunk_size = int(data[:pos], 16) #converte o tamanho do chunk de hexa p/ decimal
    print (f'Tamanho do chunked encontrado: {chunk_size}bytes')
    
    result = b"" #armazena os dados recebidos 
    ...
    while chunk_size != 0:                #continua lendo os chunks ate encontrar marcador fim
        ...
        print(f'Tamanho do proximo chunked: {chunk_size}bytes')  # o tamanho em 0 representa que acabaram os dados 
   
    fd = open("teste.txt", "wb")
    fd.write(result)
    fd.close()
